...
async function api(path,body={}){
 const context=key(),payload=JSON.stringify({platform:'apple',profileId:settings.activeId,appId:settings.current.appId,...body});
 for(let i=0;i<2;i++){
  ...
  if(!r.ok)throw Error(data.error||'请求失败');if(context!==key())throw Error('苹果项目已切换，已忽略原项目的响应');return data;
 }
}
function bind(id,fn){$(id).onclick=()=>run(fn);}
async function run(fn){if(working)return;try{await fn();}catch(e){fail(e);}}
async function job(fn){const context=key(),controls=[...document.querySelectorAll('button,select,input,textarea')].map(x=>[x,x.disabled]);working=true;controls.forEach(([x])=>x.disabled=true);status('正在处理…');try{const result=await fn();if(context!==key())throw Error('苹果项目已切换，已忽略原项目的操作结果');return result;}finally{working=false;controls.forEach(([x,disabled])=>{if(x.isConnected)x.disabled=disabled;});render();}}
...
function openSettings(isNew=false){
 ...
 ['profileName','应用配置名称',p.name],['profileApp','App Apple ID',p.appId],['profileBundle','Bundle ID',p.bundleId],['profileKey','Key ID',p.keyId],['profileIssuer','Issuer ID',p.issuerId]
  ...
  const f=$('privateKey').files[0];if(f?.size>65536)throw Error('.p8 文件超过 64KB');const body={id:p.id||'',name:$('profileName').value,appId:$('profileApp').value.trim(),bundleId:$('profileBundle').value.trim(),keyId:$('profileKey').value.trim(),issuerId:$('profileIssuer').value.trim(),privateKey:''};persist();settings=await job(async()=>{body.privateKey=f?await f.text():'';return api('config/save',body);});restore();close();if(workspace)await workspace.profileSaved(settings.activeId);status('App Store 应用设置已保存');
 }}]);
}
...
function importDialog(){
 const fields=[
  ['productId','商品 ID','必填；最多 100 字符，仅字母、数字、点、下划线、连字符。创建后不可改。'],
  ['name','参考名称','必填；最多 64 字符，仅供后台识别。'],
  ['inAppPurchaseType','内购类型','必填；CONSUMABLE（消耗型）或 NON_CONSUMABLE（非消耗型）。'],
  ['reviewNote','审核备注','选填；最多 4000 字符。同商品各语言行保持一致。'],
  ['locale','语言代码','如 zh-Hans（简体中文）、en-US（美式英语）、zh-Hant、ja。'],
  ['displayName','显示名称','填写语言时必填；用户看到的名称，2–30 字符。'],
  ['description','商品描述','填写语言时必填；说明商品用途，最多 45 字符。'],
  ['territory','价格基准地区','设置初始价格时必填；Apple 三位代码，如 USA。不是销售地区列表。'],
  ['currency','币种','设置初始价格时必填；需与基准地区匹配，如 USD。'],
  ['price','初始价格金额','设置初始价格时必填；如 0.99，不带币种符号，最多两位小数，须匹配 Apple 档位。']
 ];
 modal('导入苹果 CSV','<p>模板包含消耗型与非消耗型商品的中英文、审核备注及 USA / USD 初始价格示例。请将示例替换成自己的商品，删除不需要的示例行。</p><p>每个商品、每种语言一行；同一商品各行的 ID、参考名称、类型、审核备注和价格必须一致。保留英文表头，保存为 UTF-8 CSV。</p><label class="field">CSV 文件<input type="file" accept=".csv" id="csvFile"></label><p class="help">不会修改未列出的商品和语言。价格三列须一起填写或全留空；全留空会保留已有的本机价格草稿，撤销价格请在商品编辑器取消勾选。</p><details id="csvFieldHelp"><summary>查看全部 10 列字段说明与填写规则</summary>'+fields.map(([field,label,help])=>'<p><code>'+field+'</code> · <b>'+label+'</b><br>'+help+'</p>').join('')+'<p>仅修改基础信息时，locale、displayName、description 可一起留空；新商品请填写至少一种语言。省略 reviewNote 列会保留已有备注，保留该列但留空会清空备注。</p><p>初始价格仅适用于没有价格计划的商品。新商品创建后才能匹配价格档位；金额无匹配档位时，商品可能已创建，需核对结果后继续处理。</p><p>本模板不设置销售地区、审核截图、订阅、已有价格计划调价或送审；请在 App Store Connect 完成。</p></details>',[{label:'下载模板',run:async()=>{
  const products=[
   {...newProduct(),productId:'coins_100',name:'金币小礼包',reviewNote:'进入游戏商店，购买后获得100金币。',localizations:[{locale:'zh-Hans',name:'100金币',description:'购买后获得100金币'},{locale:'en-US',name:'100 Coins',description:'Receive 100 coins'}],initialPrice:{territory:'USA',currency:'USD',price:'0.99'}},
   {...newProduct(),productId:'remove_ads',name:'永久去广告',inAppPurchaseType:'NON_CONSUMABLE',reviewNote:'进入设置页购买，永久移除横幅广告。',localizations:[{locale:'zh-Hans',name:'永久去广告',description:'一次购买，永久移除横幅广告'},{locale:'en-US',name:'Remove Ads',description:'Permanently remove banner ads'}],initialPrice:{territory:'USA',currency:'USD',price:'2.99'}}
  ];download('apple-products-template.csv',(await job(()=>api('export',{products}))).csv);status('模板已下载');
 }},{label:'取消',run:close},{label:'检查导入',class:'primary',run:async()=>{
  const f=$('csvFile').files[0];if(!f)throw Error('请选择 CSV 文件');if(f.size>2*1024*1024)throw Error('CSV 超过 2MB，请分批');const result=await job(async()=>api('import',{csv:await f.text(),existing:clone(draft)}));status('CSV 检查完成，请核对导入草稿');modal('核对导入草稿','<p>将合并 '+result.products.length+' 个商品，确认后仅保存到本机。</p><pre>'+esc(JSON.stringify(result.products,null,2))+'</pre>',[{label:'返回',run:importDialog},{label:'应用到本机草稿',class:'primary',run:()=>{replace(result.products);close();status('导入草稿完成，尚未上传');}}]);
 }}]);
}
...
function sameSnapshot(a,b){const stable=v=>Array.isArray(v)?v.map(stable):v&&typeof v==='object'?Object.fromEntries(Object.keys(v).sort().map(k=>[k,stable(v[k])])):v;return JSON.stringify(stable(a))===JSON.stringify(stable(b));}
function applyVerified(results,targets=[]){
 for(const r of results)if(r.status==='verified'&&r.actual){
  const current=draft.find(p=>p.productId===r.productId),target=r.target||targets.find(p=>p.productId===r.productId);if(!current||!target)continue;
  const unchanged=sameDraft(current,target),baseline=old(r.productId);
  if(!unchanged&&!sameSnapshot(baseline,r.actual)&&!(Object.hasOwn(r,'before')&&sameSnapshot(baseline,r.before||null))){r.localConflict=true;continue;}
  base=base.filter(x=>x.productId!==r.productId).concat(r.actual);
  if(unchanged)draft=draft.filter(x=>x.productId!==r.productId).concat(editable(r.actual));
  else if(current.initialPrice&&target.initialPrice&&sameSnapshot(current.initialPrice,target.initialPrice))delete current.initialPrice;
 }
 persist();render();
}
...
async function reconcile(logFile){
 ...
 const html=(result.results.some(r=>r.localConflict)?'<p class="error-box">部分商品的本机快照与该历史操作不一致，已保留本机草稿和快照。请先导出草稿，再读取远端并合并修改。</p>':'')+'<p>与原目标一致的本机草稿已清除待上传标记；之后另做的本机修改会保留。未完成项可核对当前值和原目标，再编辑剩余内容。</p>'+result.results.map(r=>'<details><summary>'+esc(r.productId)+' · '+esc(resultLabel(r.status))+'</summary><p>'+esc(r.message||'')+'</p>'+(!r.canLoadCurrent?'<p>当前值未读取成功，本机草稿保留。</p>':'')+'<pre>'+esc(JSON.stringify({当前:r.actual,原目标:r.target},null,2))+'</pre></details>').join('');
...
async function history(){const data=await job(()=>api('history'));status('操作记录读取完成');modal('苹果操作记录',data.operations.length?data.operations.map((o,i)=>'<div class="option-box">'+esc(o.startedAt)+' · '+o.results.length+' 个结果 <button data-history="'+i+'">核对结果</button></div>').join(''):'<p>当前 App Store 应用暂无操作记录。</p>',[{label:'关闭',run:close}]);document.querySelectorAll('[data-history]').forEach(x=>x.onclick=()=>run(()=>reconcile(data.operations[Number(x.dataset.history)].file)));}
async function init(){
 ...
 bind('export',async()=>{
  const products=draft.filter(p=>!selected.size||selected.has(p.productId)),remoteProductIds=products.filter(p=>old(p.productId)).map(p=>p.productId);
  const result=await job(()=>api('export',{products,remoteProductIds}));download('apple-products.csv',result.csv);status('已导出商品及基准价格（优先使用本机待设置价格）；不含其他地区或未来调价计划'+(result.warnings?.length?'。'+result.warnings.join('；'):''));
 });
