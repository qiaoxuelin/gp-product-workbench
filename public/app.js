...
// Sort display rows only; retain source indices for editing and unchanged payloads.
const priceDisplayPriority=r=>!r.regionCode?3:r.price?.currencyCode==='USD'?(r.regionCode==='US'?0:1):2;
const displayPriceRows=rows=>rows.map((row,index)=>({row,index})).sort((a,b)=>priceDisplayPriority(a.row)-priceDisplayPriority(b.row));
let workspace=null,featureDirty=false;
function featurePage(title,body){featureDirty=false;const panel=$('featurePanel');panel.innerHTML='<div class="feature-heading"><span class="eyebrow">GOOGLE PLAY</span><h2>'+esc(title)+'</h2></div><div class="feature-body">'+body+'</div>';panel.oninput=()=>{featureDirty=true;};panel.onchange=()=>{featureDirty=true;};}
window.addEventListener('beforeunload',event=>{if(featureDirty||working){event.preventDefault();event.returnValue='';}});
function canLeaveFeature(){if(!featureDirty)return true;if(!confirm('当前功能页有未保存的设置，离开将放弃这些修改。继续吗？'))return false;featureDirty=false;return true;}
...
function rememberVisit(){if(mode==='demo')return;try{localStorage.setItem(VISIT_KEY,JSON.stringify({mode,projectId:settings.activeId}));}catch{status('无法保存上次访问项目，请检查浏览器存储权限',true);}}
async function restoreVisit(){
  let visit=null;try{visit=JSON.parse(localStorage.getItem(VISIT_KEY)||'null');}catch{}
  mode=new URL(location.href).searchParams.get('demo')==='1'?'demo':'live';
...
function renderHeader(){
  ...
  $('modeBanner').innerHTML=mode==='demo'?'<b>演示工作区</b><span>示例商品保存在本机，所有操作均不调用 Google 商品写入接口。</span><button id="connectLink">退出演示 →</button>':'<b>Google Play 应用 · '+esc(settings.current.name||'未配置')+'</b><span>草稿按项目和平台保存，预览确认后提交。</span><button id="connectLink">平台应用与授权 →</button>';
  bind('connectLink',()=>workspace?(mode==='demo'?workspace.exitDemo():workspace.configure()):openSettings());syncActions();
}
function render(){
  ...
  $('products').innerHTML=list.map(p=>{
    ...
    return '<tr class="'+(selected.has(p.productId)?'selected':'')+'"><td><input type="checkbox" aria-label="选择 '+esc(p.productId)+'" data-select="'+esc(p.productId)+'" '+(selected.has(p.productId)?'checked':'')+'></td><td><strong>'+esc(p.listings[0]?.title||p.productId)+'</strong><small>'+esc(p.productId)+'</small></td><td><span class="pill">'+p.purchaseOptions.length+' 个选项</span><small>'+esc(o?.purchaseOptionId||'')+' · '+p.listings.length+' 种语言</small></td><td>'+displayPriceRows(price).slice(0,2).map(({row:r})=>'<div class="price-line"><span>'+esc(r.regionCode)+'</span>'+esc(r.price?.currencyCode)+' '+esc(decimal(r.price))+'</div>').join('')+(price.length>2?'<small>共 '+price.length+' 个地区</small>':'')+'</td><td><span class="pill '+(s==='ACTIVE'?'green':'')+'">'+esc((Object.values(states[p.productId]||{}).length?'待提交 · ':'')+({ACTIVE:'已启用',DRAFT:'草稿',INACTIVE:'已停用',INACTIVE_PUBLISHED:'已停用 · 兼容',MIXED:'多种状态'}[s]||s))+'</span></td><td>'+(dirty(p)?'<span class="pill orange">'+(old(p.productId)?'待更新':'待创建')+'</span>':'<span class="pill" title="本地与上次读取的配置一致，不表示刚刚向 Google 提交了修改">无待提交修改</span>')+'</td><td><button data-edit="'+esc(p.productId)+'">编辑</button></td></tr>';
  }).join('');
...
function openSettings(id=settings.activeId){
  ...
  modal('Google Play 应用与授权','<p class="help">配置当前项目的 Google Play 应用。包名和服务账号仅用于此平台，App Store 授权单独管理。</p><div class="form-grid"><div class="wide profile-picker" hidden><label class="field">选择配置<select id="profileEdit"><option value="">＋ 新增项目</option>'+settings.profiles.map(x=>'<option value="'+esc(x.id)+'" '+(x.id===id?'selected':'')+'>'+esc(x.name)+'</option>').join('')+'</select></label><button id="addProject" type="button">＋ 新建项目</button></div><label class="field">应用配置名称<input id="profileName" value="'+esc(p.name)+'" placeholder="例如：游戏 A · 正式"></label><label class="field">Android 应用包名<input id="profilePackage" value="'+esc(p.packageName)+'" placeholder="com.company.game"></label><div class="wide"><div class="section-title">服务账号凭据</div><div id="credentialStatus">'+credentialStatus+'</div><p class="help">留空保留原凭据。选择新文件或填写新 JSON 并保存，即替换此 Google Play 应用的凭据；也可先保存应用配置，稍后再导入。</p><label class="field">'+(p.hasCredential?'选择替换用 JSON 文件':'选择 Google 服务账号 JSON 文件')+'<input id="credentialFile" type="file" accept=".json,application/json"></label><details><summary>或直接粘贴 JSON 内容</summary><label class="field">服务账号 JSON<textarea id="credentialJson" spellcheck="false" autocomplete="off" placeholder="粘贴 Google 下载的完整 JSON，保存后不回显"></textarea></label></details><p id="credentialSelected" class="help"></p></div></div>'+AUTH_GUIDE_HTML,[
    {label:'取消',run:close},{label:'保存 Google Play 设置',class:'primary',run:async()=>{
      ...
      persist();settings=await job(()=>api('config/save',body),'正在加密保存 Google Play 应用凭据…');
      $('credentialJson').value='';$('credentialFile').value='';
      mode='live';$('mode').value=mode;restore();close();if(workspace)await workspace.profileSaved(settings.activeId);status('已切换到 '+settings.current.name+'。'+(settings.current.hasCredential?'凭据已保存，可点击“读取商品”测试连接。':'可稍后在平台应用与授权中导入凭据。'));
    }}]);
...
async function history(){
  ...
  data.operations=data.operations.filter(o=>o.mode===mode&&o.packageName===(mode==='demo'?'com.example.demo':settings.current.packageName));
...
function financeDialog(){
  ...
  featurePage('账单导出','<p class="help">下载 Google Earnings 收入报告，保留收入、退款、Google 费用、税费等原始交易行，方便财务核对。</p>'+
  ...
  const saveConfig=async()=>{
    settings=await api('finance/config',{mode:'live',bucket:$('financeBucket').value});
    $('financeBucket').value=settings.current.financialBucket;featureDirty=false;renderHeader();
  };
  ...
  bind('financeList',async()=>{
    ...
    try{
      const result=await financeJob(async()=>{await saveConfig();return api('finance/list',{mode:'live',month:$('financeYear').value+'-'+$('financeMonth').value});},'正在读取 Google 月度收入报告…');
      lastList=result;
      ...
      result.files.forEach((f,i)=>bind('financeDownload'+i,async()=>{
        ...
        const data=await financeJob(()=>api('finance/download',{mode:'live',reportId:f.id},true),'正在下载并核对原始账单，请稍候…');
...
async function feishuDialog(profileId=reviewProjectId){
  ...
  modal('飞书群通知 · '+profile.name,'<p>为当前项目配置群机器人。启用后仅发送新检测到的状态变化；首次审核查询建立基线，不补发历史状态。</p><div class="form-grid">'+
    ...
    [{label:'返回审核监控',run:close},{label:'关闭',run:close}]);
  const draw=s=>{
...
async function reviewMonitorDialog(requestedId){
  ...
  if(!profile){modal('应用审核监控','<p>请先选择项目，并在 Google Play 的“平台应用与授权”配置包名和服务账号。</p>',[{label:'关闭',run:close}]);return;}
  ...
  featurePage('审核与发布监控','<p class="feature-context">'+esc(profile.name)+' · '+esc(profile.packageName)+'</p><select id="reviewProject" hidden disabled><option value="'+esc(profile.id)+'">'+esc(profile.name)+'</option></select>'+
  ...
  const saveReview=async()=>{const result=await api('monitor/config',{mode:'live',profileId,enabled:$('reviewEnabled').checked,intervalMinutes:Number($('reviewInterval').value),tracks:$('reviewTracks').selectedValues()});savedForm=readForm();featureDirty=false;return result;};
  bind('reviewSave',async()=>{const result=await job(saveReview,'正在保存监控设置…');draw(result);status(result.enabled?'后台审核监控已开启':'后台审核监控已关闭');});
  bind('reviewCheck',async()=>{const result=await job(async()=>{await saveReview();return api('monitor/check',{mode:'live',profileId});},'正在查询 Google 审核与发布状态…');draw(result);status(result.error?'检查未成功，详情见当前页面':'审核状态已更新',!!result.error);await refreshReviewBadge();});
  ...
  const body=$('featurePanel').querySelector('.feature-body'),layout=document.createElement('div'),setup=document.createElement('section'),overview=document.createElement('section');
  layout.className='monitor-layout';setup.className='monitor-settings';overview.className='monitor-overview';
  setup.innerHTML='<h3>监控设置</h3>';overview.innerHTML='<h3>版本状态与变化</h3>';
  setup.append(body.querySelector('.form-grid'),body.querySelector('.finance-actions'));overview.append($('reviewResults'));layout.append(setup,overview);
  body.querySelector('.feature-context').after(layout);
  const help=document.createElement('details');help.className='monitor-help';help.innerHTML='<summary>监控范围与运行说明</summary><p>监控当前应用版本的审核与发布状态，首次查询建立基线。本机后台服务运行时持续检查，停止工具或电脑休眠期间暂停；这里不会自动送审或发布。</p>';
  for(const child of [...body.children])if(child!==layout&&!child.matches('.feature-context,select'))help.append(child);
  body.append(help);status('监控配置已载入');

}
async function refreshReviewBadge(){
  if(working)return;
  try{const result=await api('monitor/summary');const current=result.projects.filter(p=>p.id===settings.activeId||p.profileId===settings.activeId);const count=current.reduce((n,p)=>n+p.unread,0);$('reviewBadge').textContent=count?' · '+count:'';$('reviewMonitor').title=current.filter(p=>p.unread).map(p=>p.name+'：'+p.unread+' 条状态变化').join('\n');}catch{}
}

let systemTools;
const supportDialog=()=>systemTools.supportDialog(),updateDialog=()=>systemTools.updateDialog(),showUpdateOutcome=version=>systemTools.showUpdateOutcome(version);

async function init(){
  systemTools=SystemTools.create({esc,api,modal,close,status,job,download,demo:()=>workspace.enterDemo(),persist:()=>localStorage.setItem(key(),JSON.stringify({base,draft,states})),setWorking:value=>{working=value;},render:syncActions});
  ...
  bind('about',()=>modal('关于 PlayBatch','<h3>跨平台商品工作台</h3><p>由 <a href="https://github.com/qiaoxuelin" target="_blank" rel="noreferrer">qiaoxuelin</a> 开发</p><p>当前版本：'+esc(session.version||'未知')+'</p><p>同一项目下分别管理 Google Play 与 App Store 商品、授权和草稿。</p>',[{label:'关闭',run:close}]));settings=await api('config');await restoreVisit();restore();
  bind('reviewMonitor',reviewMonitorDialog);bind('update',updateDialog);bind('finance',financeDialog);bind('support',supportDialog);
  bind('newProject',()=>openSettings(''));bind('settings',()=>workspace?workspace.configure():openSettings());bind('refresh',refresh);bind('create',newProduct);bind('copy',copyProducts);bind('price',priceDialog);bind('import',importDialog);bind('languages',languageDialog);bind('export',exportDialog);bind('preview',preview);bind('activate',()=>changeState('ACTIVE'));bind('deactivate',()=>changeState('INACTIVE'));bind('discard',discard);bind('history',history);bind('closeModal',close);
  ...
  $('mode').onchange=async()=>{if(working)return;persist();mode=$('mode').value;restore();if(workspace)await workspace.refreshMode();status('已切换工作区');if(mode==='demo'&&!draft.length){try{await loadProducts();}catch(e){showError(e.message);}}};
  ...
  workspace=await WorkspaceShell.mount({platform:'google',canLeaveFeature,openFeature:feature=>feature==='monitor'?reviewMonitorDialog():financeDialog(),busy:()=>working,persist,profileId:()=>settings.activeId,isDemo:()=>mode==='demo',modal,close,error:e=>showError(e.message),status,
    configure:(id,name)=>{openSettings(id);if(!id)$('profileName').value=name;},
    select:async(id,initial=false)=>{const demo=initial&&mode==='demo'&&!new URL(location.href).searchParams.has('live');if(!id){if(demo)return;settings={...settings,activeId:'',current:{}};mode='live';$('mode').value=mode;base=[];draft=[];states={};selected.clear();render();return;}
      if(settings.activeId!==id){settings=await api('config/switch',{id});}if(!demo){mode='live';$('mode').value=mode;}restore();}
  });
  if(workspace.activeFeature()==='products'&&!draft.length&&(workspace.hasPlatform()||mode==='demo'))await loadProducts();else if(workspace.activeFeature()==='products'&&draft.length)status('已恢复本机草稿');
  if(mode!=='demo'){await refreshReviewBadge();setInterval(refreshReviewBadge,30000);}
