...
function financeDialog(){
  ...
  modal('月度账单导出','<p class="help">下载 Google Earnings 收入报告，保留收入、退款、Google 费用、税费等原始交易行，方便财务核对。</p>'+
    ...
    '<p class="help">报告地址按项目保存，配置一次即可，刷新或重启后仍保留。同一开发者账号的项目可使用相同地址，但需各保存一次。</p><div class="finance-actions"><button id="financeSave">保存报告地址</button><button class="primary" id="financeList">保存地址并读取该月账单</button></div>'+
...
async function init(){
  ...
  bind('about',()=>modal('关于 PlayBatch','<h3>Google Play 商品工作台</h3><p>由 <a href="https://github.com/qiaoxuelin" target="_blank" rel="noreferrer">qiaoxuelin</a> 开发</p><p>当前版本：'+esc(session.version||'未知')+'</p><p>一次性商品批量创建、地区改价、多语言与状态管理，以及月度原始账单导出。</p><p>使用流程：读取商品 → 编辑本地草稿 → 预览并提交 → 核对结果。</p><p><a href="/product-guide.html" target="_blank" rel="noreferrer">查看产品说明与使用指南 ↗</a></p><p><a href="https://github.com/qiaoxuelin/gp-product-workbench/releases/latest" target="_blank" rel="noreferrer">查看新版与更新说明 ↗</a></p>',[{label:'关闭',run:close}]));settings=await api('config');await restoreVisit();restore();
