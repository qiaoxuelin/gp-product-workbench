...
每次读取、预览或提交会先核对数字 App ID 对应的 Bundle ID。预览阶段还会通过 GET /v1/territories?limit=200 分页读取地区列表，再按地区 ID 核对初始价格币种，避免这类错误在创建商品后才出现。Apple 不支持 GET /v1/territories/{id} 单地区读取。项目切换和授权变更使旧预览失效。

## CSV

在“导入 CSV”内下载模板。模板包含消耗型金币、非消耗型去广告两个示例，每个商品都有 zh-Hans / en-US 两行文案、审核备注以及 USA / USD 初始价格（0.99 / 2.99）。使用前替换示例内容、删除不需要的示例行；示例金额仍须匹配 Apple 返回的价格档位。保存为 UTF-8，保留英文表头，每种语言一行。同一商品各行的参考名称、类型、审核备注和初始价格必须一致。

导入窗口的“查看全部 10 列字段说明与填写规则”可展开查看各字段含义、必填条件及限制。导入会拒绝未知列并列出错误表头，避免审核备注或价格被静默忽略。价格基准地区不是商品销售地区；本模板不包含销售地区或审核截图上传。

| 列 | 含义与填写要求 |
| --- | --- |
| productId | 商品 ID，必填，最多 100 字符；创建后不可改 |
| name | 后台参考名称，必填，最多 64 字符 |
| inAppPurchaseType | 必填；CONSUMABLE 或 NON_CONSUMABLE |
| reviewNote | 审核备注，选填，最多 4000 字符 |
| locale | 语言代码，如 zh-Hans、en-US |
| displayName | 用户看到的名称，填写语言时必填，2–30 字符 |
| description | 商品用途描述，填写语言时必填，最多 45 字符 |
| territory | 初始价格基准地区，Apple 三位代码，如 USA |
| currency | 基准地区对应币种，如 USD |
| price | 初始价格，如 0.99，不带币种符号，最多两位小数 |

仅更新基础信息时，语言三列可一起留空；新商品请填写至少一种语言。设置初始价格时，价格三列须一起填写。
...
导出已有远端商品时，会以只读请求查询当前基准价格，将地区、币种和金额填入价格三列；有本机待设置价格时优先导出草稿价格。尚无价格计划的商品留空并提示；有价格计划但无法确定唯一有效金额时会停止导出并报错。此 CSV 不包含其他销售地区价格或未来调价计划，不能替代完整定价备份。读取远端价格需要当前项目的 API 授权。
...
上传前检查远端快照；批次不是原子事务，每个商品和步骤可能独立成功。超时后不自动重试，可从“操作记录 → 核对结果”查看原目标与当前值。已完全一致的项目可清除待上传标记；若同商品之后另有本机修改，核对结果只在本机快照仍对应原操作或当前远端时更新快照，保留新草稿及不同的待设置价格。遇到不对应的历史快照会明确提示冲突并保留本机状态。其余项目选择载入当前值后，核对并编辑剩余内容，再生成新预览。单个商品的价格档位核对失败不会阻止其他商品核对；已读取的当前值仍可查看。读取失败的商品不提供载入操作，保留本机草稿。此操作不会覆盖 Apple 远端数据。
...
- [地区列表 API](https://developer.apple.com/documentation/appstoreconnectapi/get-v1-territories)
...
- 保存草稿、检查 CSV、下载模板、读取历史和导出期间锁定关闭/切换/重复操作，并在返回时再次核对项目身份；请求失败后恢复表单原有的禁用状态。
- 预览对与远端一致的商品返回无变化结果并清除对应待上传标记；混合批次仍保留并提交真正的修改。
- 核对成功时可更新对应快照并保留后续草稿，历史记录与本机快照不一致时显示冲突提示。
- CSV 拒绝未知表头；远端商品导出补齐当前基准价格。
- 使用隔离测试数据和模拟 Apple 接口验证；未执行真实 Apple 账号商品写入或发布安装包。

本轮最终验证：苹果后端与 HTTP 测试 15 项通过，苹果浏览器测试 12 项通过；包含跨项目延迟响应、历史快照冲突、新草稿/新价格保留、价格导出及未来调价场景。
