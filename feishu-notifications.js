...
function createNotifier({file,protect,fetchImpl=fetch,now=()=>Date.now()}){
  ...
  function status(profile){const s=current(profile);return {enabled:s.enabled,states:s.states,hasWebhook:!!s.sealed,hasSecret:s.hasSecret,deliveries:s.deliveries.slice(0,30).map(({id,status,error,at,sentAt,kind,text})=>({id,status,error,at,sentAt,kind,text}))};}
  ...
  function configure(profile,input){
    ...
    data[profile.id]={...old,enabled:input.enabled,states:[...new Set(input.states)],enabledSince:input.enabled&&!old.enabled?now():old.enabledSince,sealed,hasSecret:!!secrets.secret};
  ...
  function process(profile,events){
    return exclusive(profile.id,async()=>{
      ...
      for(const event of [...events].reverse()){
        ...
        s.deliveries.unshift({id:event.id,kind:'event',status:'pending',text:message(profile,event),at:new Date(now()).toISOString()});seen.add(event.id);
      }
  ...
  function test(profile){if(active.has(profile.id))throw Error('正在发送通知，请稍后再试');return exclusive(profile.id,async()=>{const s=current(profile);if(!s.sealed)throw Error('请先保存 Webhook');const id='test-'+crypto.randomUUID();s.deliveries.unshift({id,kind:'test',status:'pending',text:'PlayBatch · 飞书机器人测试通知\n项目：'+clean(profile.name||profile.packageName)+'\n这是一条测试消息，不代表应用已过审或发布。',at:new Date(now()).toISOString()});data[profile.id]=s;write();await deliver(profile,id);return status(profile);});}
