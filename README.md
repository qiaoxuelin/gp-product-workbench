...
推荐 CSV UTF-8（支持 BOM）。上传支持自动识别 UTF-8、带 BOM 的 UTF-16、GBK，并可手动选择 Big5 等编码。名称/描述支持带引号的逗号和换行。
...
勾选商品后点击“多语言模板”（不勾选则使用当前项目全部商品），下载 CSV UTF-8 模板。可勾选额外语言追加待填写行。模板仅含 productId、languageCode、title、description 四列。用 Excel 编辑后另存为 CSV UTF-8，上传检查并预览，再应用到草稿。未列出的语言和价格/地区/购买选项均保留。当前不支持直接上传 .xlsx。
...
状态筛选提供全部、草稿（未启用）、已启用、已停用，可与搜索及待提交组合。多购买选项按任一匹配显示，状态以当前已读取的数据为准。

提交新购买选项时可选“创建并启用”或“仅创建草稿”，启用请求在创建成功后执行并读回核对。已上传的草稿可勾选后点击启用，再预览提交。

多语言上传可选择文件编码；自动识别不是所有编码的可靠判定，预览有误时请返回切换。文件中已变成问号的内容需要从原始表格恢复，并另存为 CSV UTF-8。
